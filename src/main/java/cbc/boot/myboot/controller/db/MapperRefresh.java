...
public class MapperRefresh {
        ...
        try {
            ...
            if (oldnamespace.equals(namespace)) {
                ...
                if (resource != null) {
                    ...
                    // 写入内容
                    OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(newFile, true),
                            StandardCharsets.UTF_8);
                    String XmlContent = getXmlContent(namespace, pid);
                    writer.write(XmlContent);
                    writer.close();
                ...
                // 舊的
                Resource oldtemplate = applicationContext
                        .getResource("classpath:/mapper/lowcode/" + oldnamespace + ".xml");
                // 清除緩存
                ...
                // 解析新文件
                Resource newtemplate = applicationContext
                        .getResource("classpath:/mapper/lowcode/" + namespace + ".xml");
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (data_model_type != null && data_model_id != null && data_model_sql != null && is_cache != null) {
                if (data_model_type.equals("select")) {
                    xmlContent.append("<select id=\"" + data_model_id
                            + "\" parameterType=\"java.util.HashMap\" resultType=\"java.util.HashMap\" useCache=\""
                            + is_cache + "\">");
    ...
    private void clearMap(String nameSpace) {
        logger.info("清理Mybatis的namespace=" + nameSpace
                + "在mappedStatements、caches、resultMaps、parameterMaps、keyGenerators、sqlFragments中的缓存");
